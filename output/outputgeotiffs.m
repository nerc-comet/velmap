...
for i=1:ninsar
    ...
    filename = sprintf("%s%d%s","geotiffs-1.5-m/vel_eurasiaref_frame_",i,".tif");
...
%   outgrid = insar(i).stackmap-insarfit2(i).orbmap;
    ...
    %write vstd
    filename = sprintf("%s%d%s","geotiffs-1.5-m/vstd_",i,"vstd.geo.tif");
    %vstd(isnan(vstd))= -9999;
    geotiffwrite(filename,vstd,R);  %just rewriting what we read in to make life easier later on

    %write orbmap
    filename = sprintf("%s%d%s","geotiffs-1.5-m/orbmap_",i,".tif");
    outgrid = insarfit2(i).orbmap;
    %outgrid(isnan(outgrid))= -9999;
    geotiffwrite(filename,outgrid,R);
    
    %write atmmap
    filename = sprintf("%s%d%s","geotiffs-1.5-m/atmmap_",i,".tif");
    outgrid = insarfit2(i).atmmap;
    %outgrid(isnan(outgrid))= -9999;
    geotiffwrite(filename,outgrid,R);
    
    %write resmap
    filename = sprintf("%s%d%s","geotiffs-1.5-m/resmap_",i,".tif");
    ...
    filename = sprintf("%s%d%s","geotiffs-1.5-m/modellosmap_",i,".tif");
