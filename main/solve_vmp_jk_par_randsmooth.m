...
function [fitmodel]=solve_vmp_jk_par_randsmooth(trim,smf,gps,insar,outdir,vce,njk,ncore,permsig)
%============================================================
...
%  gpssize = size(gpsvcm,1);
  ...
  parfor jkindex = 1:njk
    ...
    %disp(bootindices)
      ...
      smfoverall = log10(smf) + randn(1,1)*permsig;
      smfvec = 10.^(smfoverall + randn(nsm,1)*permsig);
      smmat_local = smmat;
      %smmat_local = diag([10.^(log10(smf) + randn(nsm,1)*0.5)])*smmat;
     for dummyindex = 1:nsm
         smmat_local(dummyindex,:) = smmat_local(dummyindex,:)*smfvec(dummyindex);
     end
      %smoothing observation
      nbb_sm=smmat_local'*smmat_local;
...
%  fitmodel = median(fitmodel_local,2);
  sel = max(abs(fitmodel_local),[],1)<=50;
disp(sum(sel))
disp(size(fitmodel_local))
