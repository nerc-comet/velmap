function[gpsfit]=gpsfwd_novcm(trim,fitmodel,invenu,gps,outdir)
%========================================================
...
for igf=1:ngf
  ...
  fidobs=fopen(char(strcat(outdir,'gpsfit',num2str(igf),'.dat')),'w');
   for i=1:ngps
    ...
    % if obsdim==2
    %   %sel=[i;i+ngps];
    %   fprintf(fidfit,'%10.4f %10.4f %10.3f %10.3f %s\n',...
    %          igpsfit(i).lon,igpsfit(i).lat,velfit(i,1),velfit(i,2),char(igpsfit(i).staid));
    % else
    ...
    %   fprintf(fidfit,'%10.4f %10.4f %10.3f %10.3f %10.3f %s\n',...
    %          igpsfit(i).lon,igpsfit(i).lat,velfit(i,1),velfit(i,2),velfit(i,3),char(igpsfit(i).staid));
    % end
    ...
    if obsdim==2
      fprintf(fidobs,'%10.4f %10.4f %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f %10.5f %s\n',...
             igpsfit(i).lon,igpsfit(i).lat,gps(igf).site(i).vel(1),gps(igf).site(i).vel(2),velfit(i,1),velfit(i,2),std_ve,std_vn,cov_en,char(igpsfit(i).staid));
    else
      std_vu=sqrt(gps(igf).site(i).vcm(3,3));
      cov_eu=gps(igf).site(i).vcm(1,3)/std_ve/std_vu;
      cov_nu=gps(igf).site(i).vcm(2,3)/std_vn/std_vu;
      fprintf(fidobs,'%10.4f %10.4f %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f %10.3f %10.5f %10.5f %10.5f %s\n',...
             igpsfit(i).lon,igpsfit(i).lat,gps(igf).site(i).vel(1),gps(igf).site(i).vel(2),gps(igf).site(i).vel(3),velfit(i,1),velfit(i,2),velfit(i,3),std_ve,std_vn,std_vu,cov_en,cov_eu,cov_nu,char(igpsfit(i).staid));
    end
  end
  %update gpsfit
